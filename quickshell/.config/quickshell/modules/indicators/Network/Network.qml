...
Item {
    ...
    readonly property var wiredDevice: {
        for (const d of devices) if (d.type === DeviceType.Wired) return d;
        return null;
    }
    ...
    readonly property var activeWifiNetwork: {
        ...
        for (const n of wifiDevice.networks.values) if (n.connected) return n;
        return null;
    }
    ...
    readonly property string connectionName: {
        if (wiredConnected) return wiredDevice.network.name;
        if (wifiConnected) return activeWifiNetwork.name;
        return "";
    }
    ...
    readonly property string connectivityLabel: {
        switch (Networking.connectivity) {
            ...
            case NetworkConnectivity.Limited: return "Limited connectivity";
            case NetworkConnectivity.Portal: return "Login required";
    ...
    readonly property string icon: {
        if (wiredConnected) return "\ueb2f";        // lan (wired)
        if (wifiConnected) {
            if (signalStrength <= 25) return "\uebe4"; // network_wifi_1_bar
            if (signalStrength <= 50) return "\uebd6"; // network_wifi_2_bar
            if (signalStrength <= 75) return "\uebe1"; // network_wifi_3_bar
            return "\ue1ba";                            // network_wifi (full)
        }
        if (Networking.wifiEnabled) return "\uf0ef";  // signal_wifi_statusbar_not_connected
        return "\ue648";                                // wifi_off
    }
    ...
    readonly property color stateColor: {
        if (wiredConnected || wifiConnected) {
            if (Networking.connectivity === NetworkConnectivity.Limited
                || Networking.connectivity === NetworkConnectivity.Portal) return Theme.gold;
            return Theme.text;
        }
        return Theme.muted;
    }
